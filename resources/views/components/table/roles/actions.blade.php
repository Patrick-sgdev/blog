...
<div class="hs-dropdown relative inline-block [--placement:bottom-right]">
  ...
  <div
    class="hs-dropdown-menu duration z-10 mt-2 hidden divide-y divide-gray-200 rounded-lg bg-white p-2 opacity-0 shadow-2xl transition-[opacity,margin] hs-dropdown-open:opacity-100 dark:divide-gray-700 dark:border dark:border-gray-700 dark:bg-gray-800"
    ...
    @if (!in_array($value, [1, 2]))
      <div class="py-2 first:pt-0 last:pb-0">
        <a class="focus:ring-primary-main flex cursor-pointer items-center gap-x-3 rounded-md px-3 py-2 text-sm text-red-600 hover:bg-gray-100 focus:ring-2 dark:text-red-500 dark:hover:bg-gray-700"
          x-on:click="window.callEvent('showConfirmationAlert', {data: {action: 'deleteRole', role: {{ $value }} }, title: '{{ trans('Are you sure you want to delete it?') }}', text: '{{ trans('This action cannot be undone.') }}'})">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
